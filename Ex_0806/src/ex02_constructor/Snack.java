...
public class Snack {
	...
	//기본생성자
	//매개변수가 별도로 없으며, 중괄호 블록 코드가 없는 비어있는 생성자를 말한다.
	...
	//생성자 선언의 이유
	//생성자는 객체를 생성함과 동시에 객체를 초기화 할 수있다.
	//생성자를 통해 객체를 초기화 한다는 것은 필드와 메서드를 호출하는 등 객체를
	//사용하기 위해 메모리에 올린다는 의미이다. 
	//생성자를 통해 객체를 메모리에 올림과 동시에, 더 나아가 객체 멤버에 접근이 가능하므로
	//일반적인 메서드처럼 객체 멤버의 데이터를 초기화 할 수있다. 
	//메서드를 통해 매개변수에 값을 전달 할 수 있었던것 처럼 
	//생성자 역시 매개변수를 통해 필드의 값을 전달할 수 있다. 
